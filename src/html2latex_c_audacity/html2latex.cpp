...
/* HTML2LaTeX -- Converting HTML files to LaTeX
   Copyright (C) 1995-2012 Frans Faase
...
Revision history:
    120206
    - Removed old style function declaration: C99-compliant
    - added %N in option t_img string for normalized file name
    - added options tab_row_sep and tab_cell_sep
    110125
...
{   { "citenr",         NULL, 0, OPT_KIND_STR},
    ...
    { "tab_row_sep",    NULL, 0, OPT_KIND_STR},
    { "tab_cell_sep",    NULL, 0, OPT_KIND_STR},
};
...
#define opt_tab_row_sep    allopt[44].str
#define opt_tab_cell_sep   allopt[45].str
...
void init_opts()
{
    ...
    assign_str(&opt_tab_row_sep , "");
    assign_str(&opt_tab_cell_sep   , "");
...
void strcpy_safer( char * dest, char *src )
{
   strcpy( dest,  src);
}
...
void add_name(char *file, char *name, bool copied)
...
void add_ref(char *file, int ln, char *to_file, char *name)
...
int find_ref(char *file, char *name)
...
int name_repeated(char *file, char *name)
/* Checks reference to HTML file `file' with (optional) NAME `name' and 
   returns appropriate return value. */
{   file_t *tfile = the_files;
    link_t *link;    
...
int name_referenced(char *file, char *name)
...
void add_ref_by(ref_by_t **p_ref_by, char *file, int ln, char *name, link_t *href)
...
link_t *find_re_href(link_t *l, file_t *file, char *name)
...
void print_ref_bys(FILE *fout, ref_by_t *ref_by)
...
void print_links(FILE *fout, link_t *link)
...
void errors_ref_bys(FILE *fout, ref_by_t *ref_by, char *file_name, int ln, bool file_exists)
...
bool norm_URL(char *origin, char *file)
...
{   
    ...
    {   biblio_t **ref = &biblios;
        ...
        {   biblio_t *newBib = ALLOC(biblio_t);
            newBib->next = *ref;
            *ref = newBib;
            newBib->nr = biblio_nr++;
            newBib->name = SALLOC(file);
            strcpy(newBib->name,  file);
        }
...
void print_bibliography( FILE *fout )
{  biblio_t *bib;
...
//   { "img",     C_YES,  T_STYLE, LATEX_KIND_OFF, "", "" },
#define H_ISINDEX 55
...
struct 
   { int  ln;
     ...
     byte tagkind;
     byte p_tagnr;
     int closing; // was bool in original code.
   } stack[MAX_STACK];
int stack_depth = 0;
...
{   int tagkind = tags[tagnr].kind,
    ...
    {
        in_table++;
        if (0 <= in_table && in_table < 10)
        {
            table_nr_row[in_table] = 0;
            table_nr_cell[in_table] = 0;
        }
    }
    else if (tagnr == H_TR)
    {
        if (0 <= in_table && in_table < 10)
        {
            if (table_nr_row[in_table] > 0 && fout != 0)
                fputs(opt_tab_row_sep, fout);
            table_nr_row[in_table]++;
            table_nr_cell[in_table] = 0;
        }
    }
    else if (tagnr == H_TH  || tagnr == H_TD)
    {
        if (0 <= in_table && in_table < 10)
        {
            if (table_nr_cell[in_table] > 0 && fout != 0)
                fputs(opt_tab_cell_sep, fout);
            table_nr_cell[in_table]++;
        }
    }
...
{   int tagnr,
    ...
    {   latex_close = tags[p_tagnr].latex_close;
        latex_kind_cel = (tags[p_tagnr].latex_kind & LATEX_KIND_CEL)!=0;
    }
...
{    if (closing_tag)
         ...
         {   tmp_tagname[0] = '/';
             strcpy_safer(tmp_tagname + 1, tags[tagnr].name);
             return tmp_tagname;
         }
     else
...
#define LATEX_OPEN(T) \
{   latex_open(fout, freport, T, ln, html_fn, tags[T].closing != 0); }
#define LATEX_OPEN_C(T,C) \
{   latex_open(fout, freport, T, ln, html_fn, C !=0); }
...
void process_comment(char *html_fn, int ln, FILE *fout, FILE *freport, char *comment, bool first)
{
...
void scan_a_file(char *html_fn,    /* file name of HTML file */
                 FILE *fout,       /* LaTeX output file */
                 FILE *freport,    /* Error report file */
    ...
    {   
                ...
                {   bool found_is = FALSE;
                    ...
                    {   if (strlen(attr_val) > MAX_DF)
                        {   if (freport != NULL)
                            ...
                            strcpy_safer(alt, attr_val);
                    }
#if 1
                    else if (   OPEN_TAG(T_DIV) && streq(attr_name, "id")
                             && attr_val[0] != '\0')
                    {   if (strlen(attr_val) > MAX_DF)
                        {   if (freport != NULL)
                                 fprintf(freport,
       ...
       /*** indention -4 ***/
            ...
            {   /* ignore */
                ...
                LATEX_CLOSES(tagnr)
            else if (OPEN_TAG(T_DIV))
            {
                LATEX_OPEN(tagnr)
               if( a_id )
               {
                  printf("Opened div with id=%s\n", id );
                  if( streq( id, "jump-to-navx" ) )
                     no_copy++;
               }
            }
            else if (CLOSE_TAG(T_DIV))
            {
               if( a_id )
               {
                  printf("Closed div with id=%s\n", id );
                  if( streq( id, "jump-to-navx" )  )
                     no_copy--;
               }
               LATEX_CLOSES(tagnr)
            }
            else if (OPEN_TAG(T_OPTIONAL))
                LATEX_OPEN(tagnr)
            else if (CLOSE_TAG(T_OPTIONAL))
                LATEX_CLOSES(tagnr)
            else
            {   
                if (in_head && !a_name)
        ...
        {   if (active_label && !in_header && after_first)
            ...
            {   int i = 0;
                ...
                print_str(no_copy ? NULL : fout, comment, freport, html_fn,
                          ln, in_math !=0 , in_verb != 0, in_alltt != 0);
                if (ch == '\n') ln++;
            }
        }
    }
...
void accept_root_URL(char *URL, FILE *fout, FILE *freport)
{   int strlen_URL = strlen(URL);                 

    document_URL = NALLOC(char, strlen_URL + 2);
    strcpy_safer(document_URL, URL);
...
void parse_URL(FILE *fin, char *refch, char *buffer)
{
    int i = 0;
    char ch = *refch;
    while (!feof(fin) && ch == ' ')
    ...
    {   int i;
        ...
        {   char html_file[MAX_SF + 1];   
            ...
            {   char buffer[MAX_SF + 1];
                ...
                {   mapping_t *newMap = ALLOC(mapping_t);
                    bool is_link_mapping = ch == 'l';

                    ch = (char)fgetc(fin);
                    parse_URL(fin, &ch, buffer);
                    
                    newMap->prefix = SALLOC(buffer);
                    strcpy_safer(newMap->prefix, buffer);

                    parse_URL(fin, &ch, buffer);

                    newMap->replace = SALLOC(buffer);
                    strcpy_safer(newMap->replace, buffer);

                    if (newMap->prefix[0] != '\0')
                        if (is_link_mapping)
                        {   newMap->next = final_mappings;
                            final_mappings = newMap;
                        }
                        else
                        {   newMap->next = link_mappings;
                            link_mappings = newMap;
                        }
                }
                else if (ch == 'b')
                {   option_bibliography = TRUE; 
...
void scan_not_included_files(FILE *freport, bool first, bool included) 
{
    if (first)
    {   bool found = TRUE;
...
{   FILE *fin =NULL,
    ...
    if (!option_stdout)
        printf("%s: Version %s\nWritten by %s\nAdapted by %s\n\n", 
               option_chk ? "chkhtml" : "html2tex", VERSION, WRITTEN_BY, ADAPTED_BY);
    ...
    strcpy_safer(reffn, fn);
    strcat(reffn, ".ref");

    if (option_cross_ref)
    {   fopen_s(&fref,reffn, "wt");
